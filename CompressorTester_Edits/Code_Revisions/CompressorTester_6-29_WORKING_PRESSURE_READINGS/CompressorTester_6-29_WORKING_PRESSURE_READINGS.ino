        ...
        //lcd.print("Ready");
          ...
          while (state == 0)                                                  //While idle
              { 
                ...
                lcd.print(buffer1 + 3);                                        //took out pressurepinVal and replaced with buffer
                lcd.setCursor(0, 1);
                 ...
                 for (int f = 0; f <= maxFilltime; f++)
                  {
                   ...
                   /* if (pressurepinValue > 60)
                      {
                        pressurepinValue = (pressurepinValue - 10);
                    ...
                    lcd.setCursor(7, 1);
                    lcd.print("PSI:");
                    lcd.print(buffer1);                                                    //buffer replaced pressurepinvalue
                    CheckButtons();
                  }
                 state = 2;
                 break;
                  ...
                  for (int b = 0; b <= settleTime; b++)                                     //Handles the settle time counter
                    {
                      ...
                      lcd.print(buffer1);                                               //Pressurepin is the PSI value from MeasurePressure function//REPLACED WITH BUFFER
                      delay(500); 
          ...
          while (state == 3)                                                                //While measuring
            { 
              ...
              if (start == HIGH && exhaust == HIGH)                                         //If no button inputs
              {
                ...
                lcd.print("Final PSI:");
                lcd.setCursor(0, 1);
                lcd.print(buffer1);                                                     //Takes the PSI value from MeasurePressure()//TOOK OUT PRESSUREPINVALUE
