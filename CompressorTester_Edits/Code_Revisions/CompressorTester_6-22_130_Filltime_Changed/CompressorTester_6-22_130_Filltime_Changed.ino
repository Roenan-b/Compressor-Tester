...
const unsigned long fillTime = 60000;
int start;
int exhaust;
int sensorAD;                               //???
int16_t adc0;                               //???
        ...
        state1:
          ...
          while (state == 1)
              {  
               ...
               /* if (pressure >= maxPressure)                                         //CHANGED THIS LOOP-RB
                        {
                          compFail = false;
                          leakFail = false;
                          elecFail = false;
                ...
                if (start == HIGH && exhaust == HIGH)                                   //Took out the overshoot-RB
                  {
                    //digitalWrite(COMPPIN, HIGH);
                    CheckButtons();
                    delay(5000);       //350 for ViAir 16092  425 for Cartee 16092             delay for fill time changed from 425* filltime to just filltime-RB          //Changed to 60
                    digitalWrite(COMPPIN, HIGH);                      
                  } */
                ...
                /*if (start == HIGH && exhaust == HIGH)               //CHANGE THIS?  OR REFORMAT
                    {
                      ...
                      else if (pressure < 0 && pressure < maxPressure)// - 23 && overShoot == 0)          //If pressure is too low//CHANGED THE PRESSURE READING TO BELOW 0-RB
                        {
                          compFail = true;
                          elecFail = false;
                          leakFail = false;
                      ...
                      else if (pressure > maxPressure)                                 //If pressure is too high              PRESSURE TOO HIGH TAKEN OUT-RB
                        {
                          compFail = false;
                          leakFail = false;
                          elecFail = false;
...
/*void CalculateLeakRate()                                                                  //REMOVED LEAK RATE CALCULATIONS-RB
{
...
{
  //Serial.print("Raw AD: ");
  //Serial.println(ads.readADC_SingleEnded(0));                                 //SERIAL.PRINT TAKEN OUT-RB
