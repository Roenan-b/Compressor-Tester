...
float pressurepinValue = 0.0;            //NEW VARIABLES-RB
float pressurepin;
float pressure;
float fpressure;
float maxPressure;
...
void MeasurePressure()
{
  ...
  pressurepinValue = ((((voltage2 - 0.5) / 4.5) * (pressureMax - pressureMin) + pressureMin)/1.99501247)- (voltage2/1.5) -1.8;
  //char buffer[10];
  dtostrf(pressurepinValue, 4, 1, buffer);
  
  

  if (pressurepinValue >= 60)
    {
      pressurepinValue = (pressurepinValue - 2.0);
    }
        ...
        //lcd.print("Ready");
          ...
          while (state == 0)                                                  //While idle
              { 
                ...
                lcd.print("PSI: .");
                MeasurePressure();
                Serial.println(voltage);                                      //ADDED A NEW LINE
                lcd.print(buffer + 3);                                        //took out pressurepinVal and replaced with buffer
                lcd.setCursor(0, 1);
                 ...
                 for (int f = 0; f <= maxFilltime; f++)
                  {
                    ...
                    lcd.setCursor(7, 1);
                    lcd.print("PSI:");
                    lcd.print(buffer);                                                    //buffer replaced pressurepinvalue
                  }
                 state = 2;
                 break;
                  ...
                  for (int b = 0; b <= settleTime; b++)                                     //Handles the settle time counter
                    {
                      ...
                      lcd.print(buffer);                                               //Pressurepin is the PSI value from MeasurePressure function//REPLACED WITH BUFFER
                      delay(500); 
          ...
          while (state == 3)                                                                //While measuring
            { 
              ...
              if (start == HIGH && exhaust == HIGH)                                         //If no button inputs
              {
                ...
                lcd.print("Final PSI:");
                lcd.setCursor(0, 1);
                lcd.print(buffer);                                                     //Takes the PSI value from MeasurePressure()//TOOK OUT PRESSUREPINVALUE
              ...
              for (int e = 0; e <= exhaust2Time; e++)
                {
                  ...
                  lcd.setCursor(9,1);
                  lcd.print("PSI:");
                  lcd.print(buffer + 3);                //Added a new conversion for exhausting psi completely//TOOK OUT PRESSUREPINVALUE
                  delay(500);
                  if (voltage <= 0.45)        //Added a new conversion for exhausting psi completely ONLY CHANGES MADE TO THIS
                    {
                      Serial.println(voltage);
                      digitalWrite(EXHAUSTPIN, HIGH);
                      delay(1500);
