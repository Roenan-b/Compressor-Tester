                ...
                if (start == HIGH && exhaust == HIGH)                                   //Took out the overshoot-RB
                  {
                    ...
                    CheckButtons();
                    delay(maxFilltime);       //350 for ViAir 16092  425 for Cartee 16092             delay for fill time changed from 425* filltime to just filltime-RB          //Changed to 60
                ...
                if (start == HIGH && exhaust == HIGH)               //CHANGE THIS?  OR REFORMAT
                    {
                      ...
                      else if (pressure >= maxPressure);   //CHANGED THIS LOOP-RB
                        {
                          compFail = false;
                          leakFail = false;
                          elecFail = false;
...
{                                                                               //ANY ADDRESS COMMENTED OUT IS THE ORIGINAL-RB
  ...
  */
  ...
  volume = 3200;                                                                                 //VOLUME CHANGED//HARDCODED
  volume = volume / 1000;
  Serial.print("Volume: ");
  Serial.print(volume);
  Serial.println(" cm3");
...
{
  EEPROMWritelong(addr1,newmaxFilltime);                                             
